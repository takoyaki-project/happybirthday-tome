...
try {
  ...
  const read = () => evaluate(`(() => {
    ...
    return {
      ...
      cake:rect('cake'),card:(() => {const r = document.querySelector('.message-card').getBoundingClientRect();return {x:r.x,y:r.y,width:r.width,height:r.height};})(),
      ...
      wish:document.querySelector('.wish-text').textContent,
  ...
  const before99 = await read(); checkFits(before99); assert.equal(before99.candleCount,99); assert.deepEqual(before99.cake,before.cake);
  await evaluate(`document.getElementById('start').click()`); await sleep(2500);
  const after99 = await read(); checkFits(after99); assert.equal(after99.candleCount,99); assert.deepEqual(after99.cake,before.cake);
  ...
  const completed = await read(); checkFits(completed); assert.ok(!completed.bubble&&!completed.counter); assert.deepEqual(completed.cake,before.cake);
  ...
  await sleep(100);
  const denied = await read(); checkFits(denied); assert.deepEqual(denied.cake,before.cake); assert.ok(denied.visibleText.includes('マイクはオフ'));
  ...
  await writeFile(path.join(output,'layout-results.json'),JSON.stringify({renderer:'Headless Microsoft Edge (Chromium), simulated silent microphone; not physical iPhone',viewport:{width:390,height:844},before,after,before99,after99,completed,denied,errors},null,2));
